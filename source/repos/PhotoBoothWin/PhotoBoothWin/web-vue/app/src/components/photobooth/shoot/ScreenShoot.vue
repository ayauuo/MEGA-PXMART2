...
const { selectedTemplate, templates, selectTemplate, showScreen, setCaptureResults, setCaptureVideoBlob, captureResults, selectedFilter, getFilterCssForCanvas, getColorBalanceForFilter, applyColorBalance, buildFinalOutput, callHost, setTestSession } =
  usePhotobooth()
...
const pictureAreaStyle = computed(() => ({
  width: `${tp.pictureAreaWidth.value}px`,
  height: `${tp.pictureAreaHeight.value}px`,
}))
...
async function onNext() {
  ...
  // #endregion
  // 原本：先進入濾鏡模式，再按一次「下一步」才合成
  // 目前需求：拍完照按下一步就直接進入影像合成，故此函式不再切換至濾鏡畫面
}
